...
import com.ballchalu.shared.domain.create_bet.BetDetailsBundle
...
import com.ballchalu.shared.domain.match_details.*
import com.ballchalu.shared.domain.position.PositionMarketItem
import com.ballchalu.shared.domain.position.Positions
import com.ballchalu.shared.network.repository.MatchDetailsRepository
        ...
        when (market?.status?.trim()) {
            ConstantsBase.open -> {
                ...
                when (run1.id) {
                    ...
                    bwlTeamRunner?.id -> {
                        run1.betfairRunnerName = bwlTeamRunner?.betfairRunnerName
                        _bwlTeamBhaavEvent.value = Event(parseRunnerObject(run1))
                    }
                }

                when (run2.id) {
                    batTeamRunner?.id -> {
                        run2.betfairRunnerName = batTeamRunner?.betfairRunnerName
                        _batTeamBhaavEvent.value = Event(parseRunnerObject(run2))
                    }
                    bwlTeamRunner?.id -> {
